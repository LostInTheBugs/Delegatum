...
- **The browser is trusted for cryptography.** PVs are encrypted/decrypted in the page. A cross-site scripting bug in the app would defeat vault confidentiality for the unlocked session — which is why the frontend is dependency-light (no embeds, no third-party runtime scripts). **Hardening recommendation:** serve HTTP security headers (nosniff / X-Frame-Options / Referrer-Policy + a tuned CSP) at the reverse proxy — not shipped by default yet, see the operator checklist (§6).
...
- [ ] Add HTTP security headers at the reverse proxy: `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, and a Content-Security-Policy tuned for the SPA (test the PDF export and share flows before enforcing it).
